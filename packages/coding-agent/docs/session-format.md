...
Created when a session starts a fresh context window without a summary. Everything before the entry leaves active model context but remains in the transcript; the optional handoff becomes the first message in the fresh window.

```json
{"type":"context_window","id":"g7h8i9j0","parentId":"f6g7h8i9","timestamp":"2024-12-03T14:12:00.000Z","reason":"tool","handoff":"Resume at step 3"}
```

`reason` is `manual`, `tool`, `threshold`, or `overflow`. `systemMessage`, when present, checkpoints the active prompt and tool declarations at the boundary.
...
`buildContextEntries()` walks from the current leaf to the root, producing the active entry list while honoring context boundaries:

1. Collects all entries on the path.
2. If a `ContextWindowEntry` is newer than every compaction, includes only that entry and everything after it.
3. Otherwise, uses the latest `CompactionEntry` after the most recent context window:
...
2. Converts selected entries to messages:
   ...
   - `context_window` -> complete system checkpoint followed by a fresh-window handoff
